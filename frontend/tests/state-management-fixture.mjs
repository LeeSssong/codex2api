...
export function createStateFixture() {
  ...
  return async function handle(url, method = 'GET', body = {}) {
    ...
    if (path === '/accounts') {
      ...
      const filtered = accounts.filter(a => !['valid', 'available', 'missing'].includes(state) || a.state_models.some(item => item.valid && (state !== 'available' || item.available) && (!model || !config.models.includes(model) || item.model === model)) === (state !== 'missing'))
      return { accounts: filtered, page: 1, page_size: 20, total: filtered.length, summary: { total: 3, normal: 2, active: 0, abnormal: 1, banned: 1, oauth: 3, disabled: 0, rate_limited: 0, unsampled: 0 }, facets: { tags: [], email_domains: [] }, stats_state: 'ready', snapshot_at: new Date(start * 1000).toISOString(), state_summary: summary }
    }
