...
try {
  for (const device of ['desktop', 'mobile']) {
    ...
    try {
      ...
      await page.locator('#codex-route-policy').click()
      await page.getByRole('option', { name: 'BPS 模型强制专用', exact: true }).click()
      await page.locator('#codex-capability-filter').click()
      await page.getByRole('option', { name: '任意能力（含未知）', exact: true }).click()
      await page.getByText(/BPS 名单内模型只用已确认支持该精确模型的 BPS 账号/).waitFor()
      await check('key-bps-models-only')
      await page.getByRole('button', { name: '保存', exact: true }).click()
      await page.getByRole('dialog').waitFor({ state: 'hidden' })
      assert.equal(key.limits.codex_route_policy, 'basispoints_models_only')
      assert.equal(key.limits.codex_capability_filter, 'any')
      assert.deepEqual(key.allowed_group_ids, [10])
      await page.getByRole('button', { name: '编辑', exact: true }).first().click()
      await page.getByRole('tab', { name: '高级限额', exact: true }).click()
      assert.match(await page.locator('#codex-route-policy').innerText(), /BPS 模型强制专用/)
      await check('key-bps-models-reopened')
