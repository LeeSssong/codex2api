...
test("source account quality and alert pages save, trigger, inspect and paginate", async () => {
  ...
  try {
    ...
    await page.getByRole("switch", { name: "启用智能运维模块" }).click();
    await page.waitForFunction(
      () => document.querySelector('[aria-label="启用智能运维模块"]').checked,
    );
    ...
    await page.getByLabel("判卷分组", { exact: true }).click();
    await page.getByRole("option", { name: "Judges #1", exact: true }).click();
    await page.getByLabel("判卷模型", { exact: true }).fill("judge-model");
    ...
    await page.screenshot({
      path:
        (process.env.SMART_OPS_ARTIFACT_DIR || "/tmp/smart-ops-ui") +
        "/quality-desktop.png",
      fullPage: true,
    });
    await page
