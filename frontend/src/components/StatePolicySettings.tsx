...
export default function StatePolicySettings() {
  ...
  useEffect(() => {
    ...
    api.getIPv6State(controller.signal).then(data => { if (!controller.signal.aborted) { setValue(data.config.require_valid_state); setEnabled(data.config.enabled) } }).catch(err => { if (!controller.signal.aborted) setError(getErrorMessage(err)) })
  ...
  return <div className="space-y-4">
    ...
    <p className="text-xs leading-relaxed text-muted-foreground">{t('ipv6State.renewalHelp')}</p>
    ...
    <Button asChild variant="outline" size="sm"><a href="/admin/state-pool">{t('ipv6State.openManagement')}</a></Button>
  </div>
}
