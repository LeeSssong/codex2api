...
func (h *Handler) runAccountQualityRound(parent context.Context, p accountops.Plan) {
	...
	for i := range round.Results {
		...
		go func(i int) {
			defer wg.Done()
			start := time.Now()
			r := accountops.Sample{ExecutionEvidence: accountops.ExecutionEvidence{Model: p.Model}}
			output, err := func() (string, error) {
				account := h.store.TakeAccountQualityProbe(p.AccountID)
				if account == nil {
...
func (h *Handler) runAccountOpsText(parent context.Context, id int64, model, prompt, effort string, evidence ...*accountops.ExecutionEvidence) (string, error) {
	...
	writer := &qualityJobWriter{header: make(http.Header), emit: func(e testEvent) {
		...
		if len(evidence) > 0 && evidence[0] != nil {
			if d := e.CodexDiagnostics; d != nil {
				*evidence[0] = accountops.ExecutionEvidence{Upstream: d.Upstream, Model: d.Model, ResponseModel: d.ResponseModel, CredentialGeneration: d.CredentialGeneration}
			}
		}
	}}
	router := gin.New()
