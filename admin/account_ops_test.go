...
func TestAccountOpsSuccessfulTextPreservesOtherGuardState(t *testing.T) {
	db := newTestAdminDB(t)
	ctx := context.Background()
	store := auth.NewStore(db, nil, nil)
	defer store.Stop()
	...
	defer upstream.Close()
	id, err := db.InsertAccountWithCredentials(ctx, "guarded", map[string]any{"upstream_type": "openai_responses", "api_key": "test", "base_url": upstream.URL, "models": []string{"gpt-4o-mini"}}, "")
	if err != nil {
		t.Fatal(err)
	}
	account := &auth.Account{DBID: id, UpstreamType: auth.UpstreamOpenAIResponses, APIKey: "test", BaseURL: upstream.URL, Models: []string{"gpt-4o-mini"}, Status: auth.StatusError, ErrorMsg: "token_guard:verified_auth_failure", Disabled: 1, CooldownReason: "manual", CooldownUtil: time.Now().Add(time.Hour)}
	store.AddAccount(account)
	before, err := db.AccountControlRevision(ctx, id)
	if err != nil {
		t.Fatal(err)
	}
	h := &Handler{db: db, store: store}
	answer, err := h.runAccountOpsText(ctx, id, "gpt-4o-mini", "q", "")
	if err != nil || answer != "21" {
		t.Fatalf("answer %q %v", answer, err)
	}
	account.Mu().RLock()
	status, detail, cooldown := account.Status, account.ErrorMsg, account.CooldownReason
	account.Mu().RUnlock()
	after, err := db.AccountControlRevision(ctx, id)
	if err != nil {
		t.Fatal(err)
	}
	if status != auth.StatusError || detail != "token_guard:verified_auth_failure" || cooldown != "manual" || atomic.LoadInt32(&account.Disabled) != 1 || before != after {
		t.Fatalf("automatic quality check cleared another restriction: status=%v error=%s cooldown=%s revision=%d->%d", status, detail, cooldown, before, after)
	}
}

func TestAccountOpsSamplesUseNativeAccountSlots(t *testing.T) {
	for _, state := range []string{"available", "busy", "dynamic_busy", "quarantined"} {
		t.Run(state, func(t *testing.T) {
			...
			upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				calls.Add(1)
				if account.GetActiveRequests() != 1 || account.GetOccupiedRequests() != 1 {
					t.Errorf("probe not counted against native slots: %d/%d", account.GetActiveRequests(), account.GetOccupiedRequests())
				}
				w.Header().Set("Content-Type", "text/event-stream")
				fmt.Fprint(w, "data: {\"type\":\"response.output_text.delta\",\"delta\":\"21\"}\n\ndata: {\"type\":\"response.completed\",\"response\":{\"status\":\"completed\"}}\n\n")
			}))
			defer upstream.Close()
			id, err := db.InsertAccountWithCredentials(ctx, "probe", map[string]any{"upstream_type": "openai_responses", "api_key": "test", "base_url": upstream.URL, "models": []string{"gpt-4o-mini"}}, "")
			if err != nil {
				t.Fatal(err)
			}
			account = &auth.Account{DBID: id, UpstreamType: auth.UpstreamOpenAIResponses, APIKey: "test", BaseURL: upstream.URL, Models: []string{"gpt-4o-mini"}, Status: auth.StatusReady}
			store.AddAccount(account)
			if state == "dynamic_busy" {
				store.SetMaxConcurrency(4)
				account.Mu().Lock()
				account.DynamicConcurrencyLimit = 1
				account.Mu().Unlock()
			}
			if state == "busy" || state == "dynamic_busy" {
			...
			if state == "quarantined" {
				...
				store.ApplyAccountEnabled(id, false)
			}
			p, err := db.SaveAccountQualityPlan(ctx, accountops.Plan{AccountID: id, Enabled: true, Model: "gpt-4o-mini", Prompt: "q", ExpectedAnswer: "21", Cron: "* * * * *", Samples: 1, Action: "disable_scheduling"})
			if err != nil {
				t.Fatal(err)
			}
			...
			pptr, err := db.ClaimAccountQualityPlan(ctx, time.Now())
			if err != nil || pptr == nil {
				t.Fatal(err)
			}
			h := &Handler{db: db, store: store, accountOps: &accountOpsRuntime{alerts: accountops.NewAccountOpsService(database.NewAccountOpsSettings(db), database.NewAccountOpsRepository(db), nil)}}
			h.accountOps.enabled.Store(true)
			h.runAccountQualityRound(ctx, *pptr)
			wantCalls := int32(1)
			wantSlots := int64(0)
			if state == "busy" || state == "dynamic_busy" {
				wantCalls = 0
			...
			if calls.Load() != wantCalls || account.GetActiveRequests() != wantSlots || account.GetOccupiedRequests() != wantSlots {
				t.Fatalf("native slot policy violated calls=%d active=%d occupied=%d", calls.Load(), account.GetActiveRequests(), account.GetOccupiedRequests())
			}
			if state == "quarantined" && atomic.LoadInt32(&account.DispatchPaused) != 1 {
				t.Fatal("probe cleared isolation")
			}
		})
	}
}
