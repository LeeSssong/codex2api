...
func (db *DB) ApplyAccountQualityOutcome(ctx context.Context, p accountops.Plan, outcome string) (action string, err error) {
	...
	err = db.WithAccountControlTx(ctx, func(tx *sql.Tx) error {
		...
		q = `SELECT status,enabled,updated_at,credentials,credential_generation FROM accounts WHERE id=$1 AND status<>'deleted' AND COALESCE(error_message,'')<>'deleted'`
		if !db.isSQLite() {
			q += ` FOR UPDATE`
		}
		var status string
		...
		if e = tx.QueryRowContext(ctx, q, p.AccountID).Scan(&status, &accountEnabled, &updated, &credentials, &generation); e != nil {
			if errors.Is(e, sql.ErrNoRows) {
				action = "account_deleted"
				return nil
			}
		...
		if outcome == "failed" {
			...
			baseline = qualityRecovery{Action: p.Action, GroupsBefore: groups, EnabledBefore: accountEnabled, PlanVersion: p.Version}
			switch p.Action {
			case "remove_groups":
			...
			baseline.ControlRevision, e = db.LockAccountControlTx(ctx, tx, p.AccountID)
			if e != nil {
				return e
			}
			...
			if status != "active" || baseline.ControlRevision == 0 || baseline.ControlRevision != revision || baseline.PlanVersion != p.Version || !slices.Equal(baseline.GroupsAfter, groups) {
		...
		if action == "restored" || action == "groups_removed" || action == "scheduling_disabled" {
			if e = db.AccountControlOutboxTx(ctx, tx, p.AccountID); e != nil {
				return e
			}
		}
...
// A quarantined account is no longer a member of removed groups. Group edits
// must still revoke that rule's recovery ownership, including same-value/ABA
// edits, so recovery cannot reattach it to an administrator-modified group.
func (db *DB) ensureAccountQualityOwnershipTriggers(ctx context.Context) error {
	statements := []string{}
	if db.isSQLite() {
		for _, event := range []string{"UPDATE", "DELETE"} {
			statements = append(statements, "CREATE TRIGGER IF NOT EXISTS account_quality_removed_group_"+event+" AFTER "+event+" ON account_groups BEGIN UPDATE accounts SET control_revision=control_revision+1 WHERE id IN (SELECT p.account_id FROM account_quality_recovery r JOIN account_quality_plans p ON p.id=r.plan_id JOIN json_each(r.snapshot,'$.removed') g WHERE g.value=OLD.id); END")
		}
	} else {
		statements = []string{
			"CREATE OR REPLACE FUNCTION invalidate_quality_removed_group_ownership() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN UPDATE accounts SET control_revision=control_revision+1 WHERE id IN (SELECT p.account_id FROM account_quality_recovery r JOIN account_quality_plans p ON p.id=r.plan_id WHERE (r.snapshot::jsonb->'removed') @> jsonb_build_array(OLD.id)); RETURN NULL; END $$",
