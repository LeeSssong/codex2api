...
func (db *DB) migrateSQLite(ctx context.Context) error {
	statements := []string{
		...
		`CREATE TABLE IF NOT EXISTS account_groups (
			...
			turn_state_inject_enabled INTEGER NOT NULL DEFAULT 0,
			created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
			updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
		);`,
	...
	columns := []struct {
		...
		{"system_settings", "turn_state_reuse_config", "TEXT NOT NULL DEFAULT '{}'"},
		{"account_groups", "turn_state_inject_enabled", "INTEGER NOT NULL DEFAULT 0"},
