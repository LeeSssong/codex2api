...
func TestAccountOpsModuleGateSuppressesDelivery(t *testing.T) {
	repo := &accountOpsRepoStub{}
	sender := &accountOpsSenderStub{}
	svc := NewAccountOpsService(&accountOpsSettingsStub{}, repo, sender)
	cfg := defaultAccountOpsConfig()
	cfg.Enabled = true
	cfg.Recipient = "ops@example.com"
	if e := svc.SaveConfig(context.Background(), cfg); e != nil {
		t.Fatal(e)
	}
	svc.SetModuleGate(func() bool { return false })
	svc.Observe(&Account{ID: 1, Platform: PlatformOpenAI}, 402, nil, []byte(`{"error":{"code":"insufficient_balance"}}`))
	if len(svc.queue) != 0 {
		t.Fatal("module off captured signal")
	}
	svc.deliverEvent(context.Background(), &AccountOpsEvent{AccountID: 1, Kind: "balance_low"})
	if sender.calls != 0 || repo.state != "suppressed" {
		t.Fatal("module off sent mail")
	}
}

func TestAccountOpsNestedStructuredBalanceOnly(t *testing.T) {
	for _, tc := range []struct{ body, want string }{{`{"errors":[{"detail":{"errorKey":"insufficient_balance"}}]}`, "balance_low"}, {`{"prompt":{"code":"insufficient_balance"}}`, ""}} {
		got, _ := classifyAccountOpsFailure(PlatformOpenAI, 402, nil, []byte(tc.body))
		if got != tc.want {
