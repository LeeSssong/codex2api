...
| `BASISPOINTS_MODELS` | 否 | `gpt-5.6-sol,gpt-6-astra` | 开启 Basispoints 时，只有列表内的模型走 BPS 通道，其余模型逐请求走原 Codex 接口（BPS 会以 `basispoints_model_access_changed` 硬拒它们）。逗号分隔，大小写不敏感，按精确名或前缀匹配（含日期快照）。设为 `*` 或 `all` 恢复「全部模型走 BPS」的旧行为 |
