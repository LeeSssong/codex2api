...
在管理后台「模型定价 → 价格同步」开启「自动更新模型与官方价格」，勾选 OpenAI 并保存间隔。默认关闭，默认间隔为每天，可设为每小时；已开启 OpenAI 官方价格轮询的部署升级后自动获得模型发现功能。

每轮先读取官方 Codex 模型目录，再按账号套餐抽样获取上游模型清单，随后同步包含新型号的官方价格。无需客户端先请求模型清单，也无需修改内置模型列表或重启服务。没有 ChatGPT 账号时仍可通过官方目录发现模型；实际调用权限由账号及上游决定。

管理员禁用的模型不会重新启用，手工价格优先于同步价。账号清单只增加模型，不因单个账号缺少权限而删除模型。官方目录来源的非内置旧条目沿用现有同步清理规则。发现或定价失败时保留已有数据，并在同步警告和服务日志中报告；未公布价格的新型号暂用现有兜底价，后续轮询获得价格后自动生效。

目前自动模型发现覆盖 Codex；xAI、Anthropic 的选项仍只控制价格更新，其他渠道模型可使用「刷新全部模型」。「立即同步官方价」保持仅同步价格，模型发现由定时轮询执行。新协议、特殊权限或特殊计费规则仍可能需要更新程序。
