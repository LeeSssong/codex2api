...
本仓库的镜像名称是 **`ghcr.io/hloolx/codex2api:latest`**，支持 `linux/amd64` 和 `linux/arm64`。包含「State 管理」、多账号多模型采集、复制迁移包和粘贴导入；完整答题复验另在「答题验证」中。
...
本次 main 更新合并官方 `de41a5e3`，保留账号级手动 Turn State 配置，并优化重新登录后的复用：

- 同一成员、工作区更新账号 JSON 后，自动管理先用新凭据验证未过期的旧 State；成功后保留原值、原采集时间和原到期时间，不因 token 更新重新采集。
- 自动管理沿用响应头验证方式；答题验证模式再次获取时，先完整复验旧值。上游返回的新 State 不会自动覆盖正在复验的原值。
- 401、403、429、网络失败和非 State 错误保留旧值，并遵守账号当前的授权、冷却和暂停状态；新的登录不会继承旧凭据的复验等待时间。
- 旧值到期、明确被上游拒绝或完整答题复验失败后，才进入替换流程。原有提前续采设置继续生效，更新 JSON 本身不改变续采周期。
- 自动管理和已验证 State 优先于手动注入；采集、复验请求不受手动注入干扰。不同成员、工作区、模型不共享 State。
...
1. 目标机器先导入同一上游账号成员和工作区。
2. 源机器进入「State 管理 → 自动管理」，点「复制全部」，或账号旁的复制按钮。
3. 目标机器进入同一页面，点「粘贴导入」并导入。确认「采集设置」包含对应账号、模型，开启「自动采集与复用」。

迁移不会续期，也不会自动打开目标机器的开关。没有任何有效值时需先配置采集出口；已有有效导入值时可以先开启复用，后续重新采集仍需可用出口。两台机器都使用新版前端才能直接处理批量包。

## 答题验证模式的迁移

以下流程适用于「答题验证」标签页，包格式与自动管理不同：

1. **目标机器准备账号。** 在目标机器的「账号管理」导入源机器所用的同一个上游账号成员和工作区，并配置目标机器可用的日常代理。两台机器的本地账号编号可以不同。
2. **源机器复制。** 打开「State 管理 → 答题验证」，在未过期条目中点击「复制有效 N 项」；只复制部分模型时先勾选，点击「复制所选」。没有有效条目时，先选账号及 Sol、Terra、Luna、Astra，点击「批量获取」，等待捕获和复验通过。
3. **目标机器粘贴。** 打开「State 管理 → 答题验证 → 导入 State」，保留迁移包模式，粘贴刚才的内容，确认自动匹配结果，点击「导入可用项」。保持「验证后自动启用」打开。任务通过后即自动复用，客户端无需手动添加 State 请求头。
